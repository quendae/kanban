...
function rewardTotal(state: GameState, playerId: PlayerId, type: PendingRewardType): number {
...
function applyGarageBenefits(player: PlayerState, benefits: readonly GarageBenefit[]): PlayerState {
  ...
  for (const benefit of benefits) {
    switch (benefit.kind) {
      case 'NONE': break;
      case 'BANKED_SHIFT': bankedShifts += benefit.amount; break;
      case 'BOOK': books += benefit.amount; break;
      case 'VOUCHER': vouchers += benefit.amount; break;
      case 'RED_SEAT': genericRedSeats += benefit.amount; break;
      case 'PP': pp += benefit.amount; break;
    }
  }
...
export function reduceEvent(state: GameState, event: GameEvent): GameState {
  switch (event.type) {
    case 'GAME_STARTED':
      return { ...state, phase: 'SELECT_DEPARTMENT', activeActorId: state.selectionOrder[state.selectionCursor] ?? null, eventIndex: state.eventIndex + 1 };
    case 'WORKSTATION_SELECTED': {
      const station = getWorkstation(event.workstationId);
      const nextCursor = state.selectionCursor + 1;
      return {
        ...state,
        players: state.players.map((player) => player.id === event.playerId ? { ...player, currentWorkstation: event.workstationId, currentDepartment: station.department, baseShiftsToday: station.shifts } : player),
    ...
    case 'WORKING_PHASE_STARTED':
      return { ...state, phase: 'WORK', workOrder: event.workOrder, workCursor: 0, activeActorId: event.workOrder[0] ?? null, eventIndex: state.eventIndex + 1 };
    case 'DESIGN_SELECTION_STARTED':
      return { ...state, activeDepartmentAction: { kind: 'DESIGN_SELECTION', playerId: event.playerId }, eventIndex: state.eventIndex + 1 };
    case 'DESIGN_TAKEN':
      return {
        ...state,
        board: { ...state.board, designs: { ...state.board.designs, [event.designId]: { kind: 'PLAYER', playerId: event.playerId, area: 'blueprints', slot: event.destinationSlot } } },
        players: state.players.map((player) => player.id === event.playerId ? { ...player, shiftsSpentToday: player.shiftsSpentToday + 1 } : player),
        pendingRewards: event.bonus === null ? state.pendingRewards : [...state.pendingRewards, { playerId: event.playerId, type: event.bonus, amount: 1 }],
        eventIndex: state.eventIndex + 1,
      };
    case 'DESIGN_SELECTION_ENDED': {
      ...
      return { ...state, board: { ...state.board, designs }, activeDepartmentAction: null, eventIndex: state.eventIndex + 1 };
    }
    case 'PARTS_COLLECTED': {
      ...
      event.partIds.forEach((partId, index) => {
        ...
        parts[partId] = { kind: 'PLAYER', playerId: event.playerId, area: 'parts', slot };
      });
      return { ...state, board: { ...state.board, parts }, players: state.players.map((player) => player.id === event.playerId ? { ...player, shiftsSpentToday: player.shiftsSpentToday + 1 } : player), eventIndex: state.eventIndex + 1 };
    }
    case 'KANBAN_ORDER_ISSUED': {
      const parts = { ...state.board.parts };
      for (const move of event.refillMoves) parts[move.partId] = { kind: 'BOARD', area: `warehouse:${move.partType}`, slot: move.destinationSlot };
      return {
        ...state,
        ...
        players: state.players.map((player) => {
          ...
          return { ...player, shiftsSpentToday: player.shiftsSpentToday + 1, kanbanOrderIssuedToday: true, kanbanOrders: event.replacementOrderId === null ? remainingOrders : [...remainingOrders, event.replacementOrderId] };
        }),
        kanbanOrderDeck: event.replacementOrderId === null ? [...state.kanbanOrderDeck, event.orderId] : [...state.kanbanOrderDeck.slice(1), event.orderId],
        pendingRewards: [...state.pendingRewards, { playerId: event.playerId, type: 'BANKED_SHIFT', amount: 1 }],
        eventIndex: state.eventIndex + 1,
      };
    }
    case 'PARTS_VOUCHER_TAKEN':
      return {
        ...state,
        players: state.players.map((player) => player.id === event.playerId ? { ...player, shiftsSpentToday: player.shiftsSpentToday + event.shiftCost, logisticsVoucherTakenToday: true } : player),
        pendingRewards: [...state.pendingRewards, { playerId: event.playerId, type: 'VOUCHER', amount: 1 }],
        eventIndex: state.eventIndex + 1,
      };
    ...
    case 'ASSEMBLY_PART_PROVIDED':
      return {
        ...state,
        board: { ...state.board, parts: { ...state.board.parts, [event.partId]: { kind: 'BOARD', area: `assembly:${event.model}`, slot: event.destinationSlot } } },
        players: state.players.map((player) => player.id === event.playerId ? { ...player, shiftsSpentToday: player.shiftsSpentToday + 1 } : player),
        eventIndex: state.eventIndex + 1,
      };
    case 'ASSEMBLY_CAR_CHAIN_RESOLVED': {
      ...
      return { ...state, board: { ...state.board, cars }, players: state.players.map((player) => player.id === event.playerId ? { ...player, pp: player.pp + event.ppAwarded } : player), eventIndex: state.eventIndex + 1 };
    }
    case 'DEMAND_RED_SEAT_CONSUMED':
      return {
        ...state,
        board: { ...state.board, activeDemands: state.board.activeDemands.map((demand) => demand.demandId === event.demandId ? { ...demand, redSeatsRemaining: Math.max(0, demand.redSeatsRemaining - 1) } : demand) },
        players: state.players.map((player) => player.id === event.playerId ? { ...player, genericRedSeats: player.genericRedSeats + 1 } : player),
        eventIndex: state.eventIndex + 1,
      };
    case 'DEMANDS_REFRESHED':
      return { ...state, board: { ...state.board, activeDemands: event.activeDemands, demandDeck: event.demandDeck, demandDiscard: event.demandDiscard }, rng: event.rng, eventIndex: state.eventIndex + 1 };
    case 'CARS_CLAIMED': {
      const cars = { ...state.board.cars };
      const designs = { ...state.board.designs };
      for (const placement of event.placements) {
        if (placement.replaceCarId !== null) cars[placement.replaceCarId] = { kind: 'SUPPLY' };
        cars[placement.carId] = { kind: 'PLAYER', playerId: event.playerId, area: 'garage', slot: placement.garageSlot };
      }
      ...
      return {
        ...state,
        board: { ...state.board, cars, designs, paceCarPosition: event.paceCarPosition },
        players: state.players.map((player) => player.id === event.playerId ? applyGarageBenefits({ ...player, shiftsSpentToday: player.shiftsSpentToday + event.shiftCost }, event.garageBenefits) : player),
        eventIndex: state.eventIndex + 1,
      };
    }
    ...
    case 'RECYCLING_PART_SWAPPED':
      return {
        ...state,
        board: { ...state.board, parts: { ...state.board.parts, [event.outgoingPartId]: { kind: 'BOARD', area: 'recycling', slot: event.recyclingSlot }, [event.incomingPartId]: { kind: 'PLAYER', playerId: event.playerId, area: 'parts', slot: event.playerSlot } } },
        eventIndex: state.eventIndex + 1,
      };
    case 'TRAINING_ADVANCED':
      return {
        ...state,
        players: state.players.map((player) => player.id === event.playerId ? {
          ...player,
          training: { ...player.training, [event.department]: event.toLevel },
          shiftsSpentToday: player.shiftsSpentToday + event.shiftCost,
          books: player.books - event.bookCost,
        } : player),
        board: { ...state.board, trainingTieOrder: { ...state.board.trainingTieOrder, [event.department]: event.tieOrder } },
        eventIndex: state.eventIndex + 1,
      };
    case 'PLAYER_CERTIFIED':
      return {
        ...state,
        players: state.players.map((player) => player.id === event.playerId ? {
          ...player,
          certifications: player.certifications.includes(event.department) ? player.certifications : [...player.certifications, event.department],
          certificationPosition: event.certificationPosition,
          conferenceSeatsFaceDown: player.conferenceSeatsFaceDown + (event.administrationSeatUnlocked ? 1 : 0),
        } : player),
        eventIndex: state.eventIndex + 1,
      };
    ...
    case 'DAY_ENDED':
      return {
        ...
        players: state.players.map((player) => ({
          ...player,
          bankedShifts: Math.max(0, player.bankedShifts - usedBankedShifts(player.baseShiftsToday, player.shiftsSpentToday)) + rewardTotal(state, player.id, 'BANKED_SHIFT'),
